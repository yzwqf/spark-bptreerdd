...
import org.apache.spark.examples.indexSpark.BplusHadoopLoader.BplusHadoopMapPartitionsWithSplitRDD
import org.apache.spark.examples.indexSpark.bptree.BPlusTree
import org.apache.spark.internal.Logging
import org.apache.spark.internal.config._
import org.apache.spark.rdd.{HadoopPartition, InputFileBlockHolder, RDD}
 ...
 *
 * @param prev The SparkContext to associate the RDD with.
 ...
 * @param initLocalJobConfFuncOpt Optional closure used to initialize any JobConf that BplusHadoopLoader
 *     creates.
 * @param inputFormatClass Storage format of the data to be read.
 * @param keyClass Class of the key associated with the inputFormatClass.
 * @param valueClass Class of the value associated with the inputFormatClass.
 * @param minPartitions Minimum number of BplusHadoopLoader partitions (Hadoop Splits) to generate.
 *
...
//  override def getPartitions: Array[Partition] = firstParent[U].partitions
  ...
  protected def getJobConf(): JobConf = {
    ...
    if (shouldCloneJobConf) {
      ...
      // disabled by default.
      BplusHadoopLoader.CONFIGURATION_INSTANTIATION_LOCK.synchronized {
      ...
      if (conf.isInstanceOf[JobConf]) {
        ...
        Option(BplusHadoopLoader.getCachedMetadata(jobConfCacheKey))
          .map { conf =>
          ...
          .getOrElse {
            ...
            // the local process. The local cache is accessed through BplusHadoopLoader.putCachedMetadata().
            ...
            BplusHadoopLoader.CONFIGURATION_INSTANTIATION_LOCK.synchronized {
              ...
              val newJobConf = new JobConf(conf)
              initLocalJobConfFuncOpt.foreach(f => f(newJobConf))
              BplusHadoopLoader.putCachedMetadata(jobConfCacheKey, newJobConf)
              newJobConf
          }
        }
      }
  ...
  override def compute(theSplit: Partition, context: TaskContext): InterruptibleIterator[(K, V)] = {
    ...
    val iter = new NextIterator[(K, V)] {
      ...
      BplusHadoopLoader.addLocalConfiguration(
        new SimpleDateFormat("yyyyMMddHHmmss", Locale.US).format(createTime),
        context.stageId, theSplit.index, context.attemptNumber, jobConf)

      reader =
        try {
          inputFormat.getIndexRecorder(split.inputSplit.value, jobConf, Reporter.NULL, postions)
  ...
  def mapPartitionsWithInputSplit[U: ClassTag](
    ...
    new BplusHadoopMapPartitionsWithSplitRDD(this, f, preservesPartitioning)
  }
  ...
  override def getPreferredLocations(split: Partition): Seq[String] = {
    ...
    val locs = hsplit match {
      case lsplit: InputSplitWithLocationInfo =>
        BplusHadoopLoader.convertSplitLocationInfo(lsplit.getLocationInfo)
      case _ => None
    }
  ...
  override def persist(storageLevel: StorageLevel): this.type = {
    if (storageLevel.deserialized) {
      logWarning("Caching BplusHadoopLoaders as deserialized objects usually leads to undesired" +
...
object BplusHadoopLoader extends Logging {
  /**
   ...
   */
  class BplusHadoopMapPartitionsWithSplitRDD[U: ClassTag, T: ClassTag](
      prev: RDD[T],
